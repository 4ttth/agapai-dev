/**
 * Minimal base64 <-> bytes helpers.
 *
 * React Native has no reliable global `btoa`/`atob` or `Buffer`, so the NaCl
 * key material (public keys, sealed boxes, nonces) is encoded by hand — the same
 * approach the Gemini Live PCM encoder uses.
 */
